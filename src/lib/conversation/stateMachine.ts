...
export async function analyseUserMessage(
  ...
  switch (state.currentStep) {
    ...
    case "lite_impact":
      // Using simple length check for now, can be updated similarly
      if (text.length > 5) {
        log('\x1b[36m%s\x1b[0m', '[StateMachine] Transitioning: lite_impact -> decision');
        return {
          type: "NEXT",
          step: "decision",
          data: { impactNarrative: text },
        };
      } else {
          log('\x1b[33m%s\x1b[0m', '[StateMachine] Staying in lite_impact (impact text too short).');
          return null;
      }
    ...
    case "details": {
      const newData: Record<string, any> = { ...state.collectedData };
      // very light heuristics (can be replaced by LLM extraction later)
      if (!newData.tools && /\b(excel|power)\b/i.test(text)) newData.tools = ["Detected tool"];
      if (!newData.frequency && /daily|weekly|monthly/i.test(text)) newData.frequency = "Mentioned";
      if (!newData.impactNarrative && /save|impact/i.test(text)) newData.impactNarrative = "Impact";

      const got = [
        Array.isArray(newData.tools) && newData.tools.length,
        newData.frequency,
        newData.impactNarrative,
      ].filter(Boolean).length;
      if (got >= 2) {
        log('\x1b[36m%s\x1b[0m', '[StateMachine] Transitioning: details -> attachments');
        return { type: "NEXT", step: "attachments", data: newData };
      }
       log('\x1b[33m%s\x1b[0m', '[StateMachine] Updating data in details state.');
      return { type: "UPDATE_DATA", data: newData };
    }
...
export function promptFor(state: ConversationState, userProfile?: any): string {
  const base = 'You are AIET-IntakeBot, a friendly assistant helping BGC employees submit automation requests. ';
  let prompt = base;
  ...
  switch (state.currentStep) {
    ...
    case "decision":
      return (
        prompt +
        "Here is a summary. Ask if they want to *Submit Now* or *Provide More Details*."+
        " Wait for their decision."
      );
    case "details":
      return prompt +
        "Great – Ask targeted questions about tools, frequency and expected impact.";
    case "attachments":
